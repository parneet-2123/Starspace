...
spacexplore::spacexplore(shared_ptr<Args> args)
  : args_(args)
  , dict_(nullptr)
  , parser_(nullptr)
  , trainData_(nullptr)
...
void spacexplore::initDataHandler() {
  if (args_->isTrain) {
    trainData_ = initData();
...
shared_ptr<InternDataHandler> spacexplore::initData() {
  if (args_->fileFormat == "fastText") {
...
void spacexplore::init() {
  cout << "Start to initialize spacexplore model.\n";
...
void spacexplore::initFromSavedModel(const string& filename) {
  cout << "Start to load a trained spacexplore model.\n";
...
void spacexplore::initFromTsv(const string& filename) {
...
Matrix<Real> spacexplore::getDocVector(const string& line, const string& sep) {
  vector<Base> ids;
  parseDoc(line, ids, sep);
  return model_->projectLHS(ids);
}

MatrixRow spacexplore::getNgramVector(const string& phrase) {
  vector<string> tokens;
...
void spacexplore::nearestNeighbor(const string& line, int k) {
...
unordered_map<string, float> spacexplore::predictTags(const string& line, int k){
    args_->K = k;
...
Metrics spacexplore::evaluateOne(
...
void spacexplore::printDoc(ostream& ofs, const vector<Base>& tokens) {
  for (auto t : tokens) {
    // skip ngram tokens
...
string spacexplore::printDocStr(const vector<Base>& tokens) {
  for (auto t : tokens) {
    if (t.first < dict_->size()) {
...
void spacexplore::saveModel(const string& filename) {
...
void spacexplore::saveModelTsv(const string& filename) {
