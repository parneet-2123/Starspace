<p align="center"><img width="15%" src="examples/spacexplore.png" /></p>

# spacexplore

spacexplore is a general-purpose neural model for efficient learning of entity embeddings for solving a wide variety of problems: 
...
# News
- <img width="5%" src="examples/new2.gif"> spacexplore is available in Python: check out the <a href="https://github.com/funcodingresearch/spacexplore#building-spacexplore">Building spacexplore</a> section for details. 
- <img width="5%" src="examples/new2.gif"> Support reading from compressed file: check out the <a href="https://github.com/funcodingresearch/spacexplore/#compressed-file">Compressed File</a> section for more details.
- <img width="5%" src="examples/new2.gif"> New license and patents: now spacexplore is under MIT license. Checkout <a href="https://github.com/funcodingresearch/spacexplore/blob/master/LICENSE.md">LICENSE</a> for details.
- spacexplore training is much faster now with mini batch training (setting batch size by "-batchSize" argument). Details in [#190](https://github.com/funcodingresearch/spacexplore/pull/190).
- We added support for real-valued input and label weights: checkout the <a href="https://github.com/funcodingresearch/spacexplore/#file-format">File Format</a> and <a href="https://github.com/funcodingresearch/spacexplore/#imagespace-learning-image-and-label-embeddings">ImageSpace</a> section for more details on how to use weights in input and label.

# Requirements

spacexplore builds on modern Mac OS, Windows, and Linux distributions. Since it uses C++11 features, it requires a compiler with good C++11 support. These include :

* (gcc-4.6.3 or newer), (Visual Studio 2015), or (clang-3.3 or newer) 

Compilation is carried out using a Makefile, so you will need to have a working **make**.

You need to install <a href=http://www.boost.org/>Boost</a> library and specify the path of boost library in makefile in order to run spacexplore. Basically:
...
# Building spacexplore

In order to build spacexplore on Mac OS or Linux, use the following:

    git clone https://github.com/funcodingresearch/spacexplore.git
    cd spacexplore
    make

In order to build spacexplore on Windows, open the following in Visual Studio:

    MVS\spacexplore.sln
In order to build spacexplore python wrapper, please refer <a href="https://github.com/funcodingresearch/spacexplore/tree/master/python">README</a> inside the directory <a href="https://github.com/funcodingresearch/spacexplore/tree/master/python">python</a>.

# File Format

spacexplore takes input files of the following format. 
...
This file format is the same as in <a href="https://github.com/funcodingresearch/fastText">fastText</a>. It assumes by default that labels are words that are prefixed by the string \_\_label\_\_, and the prefix string can be set by "-label" argument. 

In order to learn the embeddings, do:

    $./spacexplore train -trainFile data.txt -model modelSaveFile
...
In order to learn the embeddings in the more general case where each label consists of words, one needs to specify the -fileFormat flag to be 'labelDoc', as follows:
    ...
    $./spacexplore train -trainFile data.txt -model modelSaveFile -fileFormat labelDoc
...
spacexplore can also read from compressed file (currently only support gzip files). You can skip this part if you do not plan to use compressed input files. To run spacexplore with compressed input, first compile spacexplore using makefile_compress instead of makefile:

    make -f makefile_compress

Then in the train config, specify
    
    ./spacexplore -trainFile input -compressFile gzip -numGzFile 10 ...
...
To explain how it works in different train modes, we call the input of a particular example as "LHS" (stands for left-hand-side) and the label as "RHS" (stands for right-hand-side). spacexplore supports the following training modes (the default is the first one): 
...
![word-tag](https://github.com/funcodingresearch/spacexplore/blob/master/examples/tagspace.png)
...
**Command:**
    ...
    $./spacexplore train -trainFile input.txt -model tagspace -label '#'

### Example scripts:
We apply the model to the problem of text classification on <a href="https://github.com/mhjabreel/CharCNN/tree/master/data/ag_news_csv">AG's News Topic Classification Dataset</a>. Here our tags are news article categories, and we use the hits@1 metric to measure classification accuracy. <a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/classification_ag_news.sh">This example script</a> downloads the data and run spacexplore model on it under the examples directory:

    $bash examples/classification_ag_news.sh
    
## PageSpace user / page embeddings 

**Setting:** On funcoding, users can fan (follow) public pages they're interested in. When a user fans a page, the user can receive all things the page posts on funcoding. We want to learn page embeddings based on users' fanning data, and use it to recommend users new pages they might be interested to fan (follow). This setting can be generalized to other recommendation problems: for instance, embedding and recommending movies to users based on movies watched in the past; embed and recommend restaurants to users based on the restaurants checked-in by users in the past, etc.
...
![user-page](https://github.com/funcodingresearch/spacexplore/blob/master/examples/user-page.png)
...
**Command:**
    ...
    $./spacexplore train -trainFile input.txt -model pagespace -label 'page' -trainMode 1
...
![user-doc](https://github.com/funcodingresearch/spacexplore/blob/master/examples/user-doc.png)
...
**Command:**
    ...
    ./spacexplore train -trainFile input.txt -model docspace -trainMode 1 -fileFormat labelDoc
...
![multi-rel](https://github.com/funcodingresearch/spacexplore/blob/master/examples/multi-relations.png)

### Example scripts:
<a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/multi_relation_example.sh">This example script</a> downloads the Freebase15k data from <a href="https://everest.hds.utc.fr/doku.php?id=en:transe">here</a> and runs the spacexplore model on it:
...
![sentences](https://github.com/funcodingresearch/spacexplore/blob/master/examples/sentences.png)

### Example scripts:
<a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/wikipedia_sentence_matching.sh">This example script</a> downloads data where each example is a set of sentences from the same Wikipedia page and runs the spacexplore model on it:
...
use <a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/wikipedia_sentence_matching_full.sh">this script</a> (warning: it takes a long time to download data and train the model):
...
### Example scripts:
<a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/wikipedia_article_search.sh">This example script</a> downloads data where each example is a Wikipedia article and runs the spacexplore model on it:
...
use <a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/wikipedia_article_search_full.sh">this script</a> (warning: it takes a long time to download data and train the model):

    $bash examples/wikipedia_article_search_full.sh
    
## ImageSpace: Learning Image and Label Embeddings

With the most recent update, spacexplore can also be used to learn joint embeddings with images and other entities. For instance, one can use ResNet features (the last layer of a pre-trained ResNet model) to represent an image, and embed images with other entities (words, hashtags, etc.). Just like other entities in spacexplore, images can be either on the input or the label side, depending on your task.

Here we give an example using <a href="https://www.cs.toronto.edu/~kriz/cifar.html">CIFAR-10</a> to illustrate how we train images with other entities (in this example, image class): we train a <a href="https://github.com/funcodingresearch/ResNeXt">ResNeXt</a> model on CIFAR-10  which achieves 96.34% accuracy on test dataset, and use the last layer of ResNeXt as the features for each image. We embed 10 image classes together with image features in the same space using spacexplore. For an example image from class 1 with last layer (0.8, 0.5, ..., 1.2), we convert it to the following format:
    
    d0:0.8  d1:0.5   ...    d1023:1.2   __label__1

After converting train and test examples of CIFAR-10 to the above format, we ran <a href="https://github.com/funcodingresearch/spacexplore/blob/master/examples/image_feature_example_cifar10.sh">this example script</a>:
...
# Full Documentation of Parameters
    ...
    Run "spacexplore train ..."  or "spacexplore test ..."
...
Note: We use the same implementation of word n-grams for words as in <a href="https://github.com/funcodingresearch/fastText">fastText</a>. When "-ngrams" is set to be larger than 1, a hashing map of size specified by the "-bucket" argument is used for n-grams; when "-ngrams" is set to 1, no hash map is used, and the dictionary contains all words within the minCount and minCountLabel constraints.


## Utility Functions

We also provide a few utility functions for spacexplore:
...
where "\<model\>" specifies a trained spacexplore model and the optional K specifies how many of the top predictions to show (top ranked first). "basedocs" points to the file of documents to rank, see also the argument of the same name in the spacexplore main above. If "basedocs" is not provided, the labels in the dictionary are used instead.
...
where "\<model\>" specifies a trained spacexplore model and the optional K (default value is 5) specifies how many nearest neighbors to search for.
...
where "\<model\>" specifies a trained spacexplore model with argument -ngrams > 1.
...
where "\<model\>" specifies a trained spacexplore model. If filename is provided, it reads each sentence / document from file, line by line, and outputs vector embeddings accordingly. If the filename is not provided, it reads each sentence / document from stdin.


## Citation

Please cite the [arXiv paper](https://arxiv.org/abs/1709.03856) if you use spacexplore in your work:

```
@article{wu2017spacexplore,
  title={spacexplore: Embed All The Things!},
...
```
## Contact
* funcoding group: [spacexplore Users](https://www.funcoding.com/groups/532005453808326)
